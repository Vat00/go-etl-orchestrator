...
┌───────────────▼───────────────┐ ┌─────────────────────────▼───────────────┐
│ │ │ │
│ Orchestrator (API) │ │ Worker(s) │
│ │ │ │
│ - POST /task │ │ - BLPop из Redis очереди │
│ - GET /task/{id} │◄──│ - Выполнение shell/HTTP задач │
│ - Сохранение в PostgreSQL │ │ - Обновление статуса в БД │
│ - Отправка ID в Redis │ │ - Автоматические ретраи (3 попытки) │
│ │ │ │
└───────────────┬───────────────┘ └─────────────────────▲─────────────────────┘
│ │
│ ┌─────────────────────┐ │
